...
bool CPicture::CreateThumbnailFromSurface(const unsigned char *buffer, int width, int height, int stride, const std::string &thumbFile)
{
  ...
  {
#if defined(TARGET_RASPBERRY_PI)
    if (CServiceBroker::GetSettings()->GetBool("videoplayer.acceleratedjpegs") && COMXImage::CreateThumbnailFromSurface(const_cast<unsigned char*>(buffer), width, height, XB_FMT_A8R8G8B8, stride, thumbFile.c_str()))
      return true;
#endif
  }
